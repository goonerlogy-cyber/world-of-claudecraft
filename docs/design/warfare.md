...
- Ravenrift battleground win: 60 Honor per winning player
  (`BATTLEGROUND_WIN_HONOR`).
- Ravenrift battleground loss, played out to a result: 20 Honor
...
25, then 0 percent. Ravenrift results decay on the same 100/50/25/0 curve per
...
Ravenrift rating is its own per-character ladder (base 1500, floor 100), moved
