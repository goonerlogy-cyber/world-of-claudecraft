...
// both terrain tiers tint toward. Split out of terrain.ts as PLAIN DATA (hex
// numbers, no Three) so the far-vista mesh (far_terrain_core.ts, a pure
// Node-tested core) colors distant ground from the exact same source the
// near splat/Lambert terrain uses, and the two can never drift apart.
...
// color recipe (hex mirrors of terrain.ts's scratch THREE.Colors).
