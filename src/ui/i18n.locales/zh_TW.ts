...
export const zh_TW: Partial<Record<TranslationKey, string>> = {
  ...
  'guide.ravenriftPage.fieldBody':
    '一片有圍牆的露天戰場,兩端各有一座要塞。每座要塞是三面圍合的工事,有兩條出路:正門前一道矮壁壘擋住直衝的路線,另有一側城牆上通往快速側翼的窄小後門。兩側城牆下各有一間前後開門的側室,牆邊還留有淺淺的掩體壁龕,讓沿牆奔跑成為一條有掩護、有伏擊角落的真正路線。中央是斷壁殘垣:錯落的矮牆、石柱、木箱與散落的殘牆碎段,在空心的中央廢墟四周織出數條不同的小徑,四枚疾行符文分布在旗幟要道與兩翼。',
