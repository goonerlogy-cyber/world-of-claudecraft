...
export const zh_TW: Partial<Record<TranslationKey, string>> = {
  ...
  'hudChrome.warfare.reasons.battlegroundWin': '鴉裂谷勝利',
  'hudChrome.warfare.reasons.battlegroundComplete': '鴉裂谷參戰',
  ...
  'hudChrome.bg.searching': '配對中。佇列 {count}/10。',
  'hudChrome.bg.queuedParty': '{count}人隊伍。',
  'hudChrome.bg.queueNote':
    '兩隊各五人。奪取敵方軍旗並送回己方要塞。先奪得5次者獲勝。可組滿5人隊伍一同排隊;拾取疾行符文,並利用掩體甩開追兵。',
  ...
  'hudChrome.bg.flagCarriedBy': '{name}攜帶中',
  ...
  'hudChrome.bg.levelRequirement': '需要等級{level}。',
  'hudChrome.bg.board.kills': '擊殺',
  'hudChrome.bg.board.deaths': '死亡',
  'hudChrome.bg.board.captures': '奪旗',
  'hudChrome.bg.flagTakenBanner': '{name}奪走了{team}的旗幟!',
  'hudChrome.bg.flagReturnedBanner': '{team}的旗幟已歸還!',
  'hudChrome.bg.capturedBanner': '{name}奪取了{team}的旗幟!{crimson}:{azure}',
  'hudChrome.bg.capturedLog': '{name}奪取了{team}的旗幟。比分{crimson}:{azure}。',
  'hudChrome.bg.victoryBanner': '勝利!鴉裂谷{crimson}:{azure}。評分{rating}({delta})',
  'hudChrome.bg.defeatBanner': '戰敗。鴉裂谷{crimson}:{azure}。評分{rating}({delta})',
  'hudChrome.bg.drawBanner': '鴉裂谷平手{crimson}:{azure}。評分{rating}({delta})',
  'hudChrome.bg.endLog': '鴉裂谷結束,{crimson}:{azure}。評分{rating}({delta})。',
  ...
  'guide.ravenriftPage.fieldBody':
    '一片有圍牆的露天戰場,被隔成三個區域:兩隊各自的前場,以及夾在中間的廢墟中庭。兩道橫貫全場的隔牆劃出邊界,穿行區域之間必須經過爭奪點:寬闊的正門、緊貼遠側城牆的窄拱門,或是騎跨在隔牆上的門樓小屋,其錯位的兩扇門逼你繞過伏擊死角。門樓設在後門一側,從要塞窄小後門溜出的旗手可以順勢鑽進門樓;每座要塞正門前還有一道矮壁壘擋住直衝的路線。中庭裡有空心的中央廢墟與兩枚側翼疾行符文,另外兩枚守在兩旗的接近路上。',
  ...
  'guide.ravenriftPage.respawnBody':
    '死亡保留經典儀式:釋放靈魂後,它會在你方主堡的圍欄墓地中升起,並被結界束縛,直到你方下一波復活潮(停留過久的屍體會在短暫寬限後自動釋放)。每波復活潮會同時喚起所有等待的靈魂,並在行動前獲得短暫保護;兩隊的復活潮刻意錯開,戰鬥永遠不會同時完全重置。這裡沒有跑屍,也沒有靈魂醫者的交易:復活潮是唯一的歸途。',
