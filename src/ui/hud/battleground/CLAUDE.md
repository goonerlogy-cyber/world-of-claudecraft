...
The Ravenrift 5v5 capture-the-flag HUD surface behind the `index.ts` barrel:
...
- `battleground_window_view.ts`: the queue panel's pure core (standing, the
  ...
  Ravenrift is that window's primary tab (`src/ui/pvp_tabs_view.ts` decides
  ...
  the merged window on the Ravenrift tab.
