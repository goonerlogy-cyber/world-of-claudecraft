...
export const guideStrings = {
  ...
  ravenriftPage: {
    ...
    fieldBody:
      "A walled, open-air field carved into three chambers: each team has its own field before its keep, and the walled Ruin Courtyard sits between them. Two curtain walls span the full width, and every move between chambers passes a contested crossing: the wide main gate, the narrow flank arch by the far rampart, or the gatehouse, a small room straddling the wall whose offset doors force a jog past an ambush corner. The gatehouse sits on the postern side, so a runner slipping out a keep through its narrow postern gap can chain straight into it, while a low barricade breaks the straight charge into each keep mouth. The courtyard holds the hollow heart ruin and the two flank Sprint Runes; the other two wait on the flag approaches. A Battle or Ward Rune (more damage dealt, or less damage taken, for a few seconds) waits at each main gate's courtyard mouth: both pads open the match on the same face and flip with every claim.",
    flagsHeading: 'Flags',
    flagsBody:
