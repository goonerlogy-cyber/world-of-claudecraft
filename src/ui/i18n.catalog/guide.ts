...
export const guideStrings = {
  ...
  ravenriftPage: {
    ...
    fieldBody:
      'A walled, open-air field with a keep at each end. Every keep is a three-sided enclosure with two ways out: the main mouth, where a low barricade breaks the straight charge, and a narrow postern gap in one side wall that opens onto the fast flank. Each flank rampart carries a pocket side room with a doorway at either end and shallow cover bays beyond it, so the wall run is a real route with ambush corners rather than a sprint in the open. The middle is a broken ruin: staggered walls, pillars, crates, and loose wall fragments thread several distinct paths around the hollow heart, and four Sprint Runes wait at the flag approaches and the two flanks.',
    flagsHeading: 'Flags',
    flagsBody:
