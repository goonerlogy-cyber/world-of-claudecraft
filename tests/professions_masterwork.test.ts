...
describe('draw-order determinism over a real Sim', () => {
  ...
  // re-hunted after the procedural-dungeons merge shifted the camp-driven
  // world-gen draw sequence) so the three-success sequence procs on the
  // second and third successful crafts; only the pinned literal is
  // committed, per the suite idiom. Spares on record: 152, 273, 304, 312,
  // and 314.
  const SEED = 90;
...
describe('proc-chance wiring over a real Sim (hunted boundary-window seeds)', () => {
  ...
  it('a self-signed reagent feeds the proc chance: the same seed procs only with the signed copy', () => {
    // Seed 7, hunted (re-hunted after the procedural-dungeons merge shifted
    // the camp-driven world-gen draw sequence): the single proc draw lands in
    // [0.03, 0.05), above the 3 percent base but under base plus the 2
    // percent signed-reagent bonus, so the proc fires ONLY when crafting.ts
    // passes the signed-reagent holding check into masterworkProcChance.
    // Spares on record: 22, 38, 112, and 120.
    const SEED = 7;
  ...
  it('the specialization threshold binds in the craft path: skill 74 misses where 75 and 76 proc', () => {
    ...
    // Seed 39, hunted (re-hunted after the procedural-dungeons merge shifted
    // the camp-driven world-gen draw sequence): the single proc draw lands in
    ...
    // under the hunted draw). Spares on record: 68, 239, 267, and 404.
    const SEED = 39;
